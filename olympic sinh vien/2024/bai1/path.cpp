...
const int MAX = 200200; // đủ lớn để chứa m+n tối đa
const int LIM = 1010;   // giới hạn cho vùng quy hoạch động (do các điểm chặn <= 1000)

// Mảng giai thừa và nghịch đảo giai thừa
int frac[MAX], finv[MAX];

// Tiền xử lý giai thừa và nghịch đảo giai thừa modulo MOD
int modPow(int x, int k) // x^k
{
    int res = 1;
    while (k > 0)
    {
