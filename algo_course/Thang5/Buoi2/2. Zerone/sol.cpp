...
int main () {
  ...
  long long ans = 0LL; 
  std::cin >> a0 >> a1 >> b0 >> b1;     
  for (long long groups = 1; groups * b0 <= a0; groups++) {
    long long groups0 = groups;
    long long groups1 = groups;
    if (1LL * groups1 * b1 <= a1) {
      (ans += (2LL * calc(a0 - groups0 * b0, groups0 + 1) * calc(a1 - groups1 * b1, groups1 + 1)) % mod) %= mod;
    }
    if (groups > 1) {                 
      groups1 = groups - 1;
      if (1LL * groups1 * b1 <= a1) { 
        (ans += (calc(a0 - groups0 * b0, groups0 + 1) * calc(a1 - groups1 * b1, groups1 + 1)) % mod) %= mod;
      }
    }  
    groups1 = groups + 1; 
    if (1LL * groups1 * b1 <= a1) {
