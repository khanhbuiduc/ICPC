...
int main () {
  ...
  for (int i = 0; i < (int) listRectangles.size() - 1; i++) {
    updateArea(1, 0, 2000000, listRectangles[i].x1, listRectangles[i].x2 - 1, listRectangles[i].value); 
    ans += (long long) (listRectangles[i + 1].y - listRectangles[i].y) * it[1].area;
  } 
  std::cout << ans;
  return 0; 
}